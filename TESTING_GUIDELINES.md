...
test/
  ...
  fixtures/
    ocsf_schema/1.9/             # vendored OCSF JSON schemas (classes + objects)
    ocsf/1.9/                    # golden fixtures, regenerated by scripts/gen_fixture.exs
```
...
Verify builder output matches expected JSON snapshots. The snapshots
live under `test/fixtures/ocsf/<major.minor>/` and are regenerated with
`mix run scripts/gen_fixture.exs` after an intentional serializer change
...
test "logon event matches golden fixture" do
  ...
  expected = "test/fixtures/ocsf/1.9/authentication/logon_success.json"
    |> File.read!()
    |> Jason.decode!()
...
test "logon JSON validates against OCSF authentication schema" do
  ...
  schema = "test/fixtures/ocsf_schema/1.9/authentication.json"
    |> File.read!()
    |> Jason.decode!()
