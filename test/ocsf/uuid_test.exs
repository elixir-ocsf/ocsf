defmodule OCSF.UUIDTest do
  ...
  test "v7/0 returns a 16-byte binary" do
    uuid = UUID.v7()
    assert byte_size(uuid) == 16
  end

  test "v7/0 has version 7 and variant 10" do
    <<_::48, version::4, _::12, variant::2, _::62>> = UUID.v7()
    assert version == 0b0111
    assert variant == 0b10
  end

  test "v7/0 is time-sortable across milliseconds" do
    uuid1 = UUID.v7()
    Process.sleep(2)
    uuid2 = UUID.v7()
  ...
  test "v7_string/0 returns a valid UUID string" do
    uuid = UUID.v7_string()
  ...
  test "v7_string/0 produces unique values" do
